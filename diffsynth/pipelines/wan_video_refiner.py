...
class WanVideoRefinerPipeline(BasePipeline):
    def __init__(self, device="cuda", torch_dtype=torch.bfloat16):
        ...
        self.units = [
            WanVideoUnit_ShapeChecker(), # 校正/对齐 height/width/num_frames
            WanVideoUnit_NoiseInitializer(), # 按分辨率和帧数生成初始噪声
            WanVideoUnit_PromptEmbedder(), # 把 prompt 编码成文本特征。
            WanVideoUnit_S2V(),
            WanVideoUnit_InputVideoEmbedder(), # 把 input_video 编码成 latents 训练模式返回噪声+输入 latents）
            ...
            WanVideoUnit_VACE(), # 即便没有 vace 视频也会返回 vace_context=None 
            WanVideoUnit_AnimateVideoSplit(), # 存在 input_video 时拆分/裁剪 animate_* 输入（这里传入的 animate_* 会被截帧）
            WanVideoUnit_AnimatePoseLatents(), # 若 animate_pose_video 非空，VAE 编码得到 pose_latents。
            WanVideoUnit_AnimateFacePixelValues(), # 若 animate_face_video 非空，生成人脸条件 face_pixel_values
    ...
    def __call__(
        ...
        # Scheduler
        self.scheduler.set_timesteps(num_inference_steps, denoising_strength=denoising_strength, shift=sigma_shift) # 20 | 1.0 | 5.0
        ...
        inputs_shared = {
            "input_image": input_image, # (1280, 720)
        ...
        # Denoise
        self.load_models_to_device(self.in_iteration_models) # ("dit", "motion_controller", "vace", "animate_adapter", "vap")
        ...
        for progress_id, timestep in enumerate(progress_bar_cmd(self.scheduler.timesteps)):
            ...
            # Timestep
            timestep = timestep.unsqueeze(0).to(dtype=self.torch_dtype, device=self.device) # torch.Size([1])
            
            # Inference
            noise_pred_posi = self.model_fn(**models, **inputs_shared, **inputs_posi, timestep=timestep) # torch.Size([1, 16, 21, 90, 160])
            if cfg_scale != 1.0:
                if cfg_merge:
                ...
                noise_pred = noise_pred_posi # torch.Size([1, 16, 21, 90, 160])

            # Scheduler
            inputs_shared["latents"] = self.scheduler.step(noise_pred, self.scheduler.timesteps[progress_id], inputs_shared["latents"])
            if "first_frame_latents" in inputs_shared: # False
...
class WanVideoUnit_InputVideoEmbedder(PipelineUnit):
    ...
    def process(self, pipe: WanVideoRefinerPipeline, input_video, noise, tiled, tile_size, tile_stride, vace_reference_image):
        ...
        input_video = pipe.preprocess_video(input_video) # torch.Size([1, 3, 81, 480, 832])
        input_latents = pipe.vae.encode(input_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device) # torch.Size([1, 16, 21, 60, 104])
        if vace_reference_image is not None:
        ...
        if pipe.scheduler.training:
            return {"latents": noise, "input_latents": input_latents} # torch.Size([1, 16, 21, 60, 104]) | torch.Size([1, 16, 21, 60, 104])
        else:
...
class WanVideoUnit_ImageEmbedderCLIP(PipelineUnit):
    ...
    def process(self, pipe: WanVideoRefinerPipeline, input_image, end_image, height, width):
        if input_image is None or pipe.image_encoder is None or not pipe.dit.require_clip_embedding:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        image = pipe.preprocess_image(input_image.resize((width, height))).to(pipe.device) # torch.Size([1, 3, 480, 832])
        clip_context = pipe.image_encoder.encode_image([image]) # torch.Size([1, 257, 1280])
        if end_image is not None:
            end_image = pipe.preprocess_image(end_image.resize((width, height))).to(pipe.device)
            if pipe.dit.has_image_pos_emb:
                clip_context = torch.concat([clip_context, pipe.image_encoder.encode_image([end_image])], dim=1)
        clip_context = clip_context.to(dtype=pipe.torch_dtype, device=pipe.device) # torch.Size([1, 257, 1280])
...
class WanVideoUnit_ImageEmbedderVAE(PipelineUnit):
    ...
    def process(self, pipe: WanVideoRefinerPipeline, input_image, end_image, num_frames, height, width, tiled, tile_size, tile_stride):
        ...
        if end_image is not None: # None
            end_image = pipe.preprocess_image(end_image.resize((width, height))).to(pipe.device)
            vae_input = torch.concat([image.transpose(0,1), torch.zeros(3, num_frames-2, height, width).to(image.device), end_image.transpose(0,1)],dim=1)
            msk[:, -1:] = 1
        else:
            vae_input = torch.concat([image.transpose(0, 1), torch.zeros(3, num_frames-1, height, width).to(image.device)], dim=1) # torch.Size([3, 81, 480, 832])
        ...
        y = pipe.vae.encode([vae_input.to(dtype=pipe.torch_dtype, device=pipe.device)], device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride)[0] # torch.Size([16, 21, 60, 104])
        y = y.to(dtype=pipe.torch_dtype, device=pipe.device) # torch.Size([16, 21, 60, 104])
        y = torch.concat([msk, y]) # torch.Size([20, 21, 60, 104])
        y = y.unsqueeze(0) # torch.Size([1, 20, 21, 60, 104])
        y = y.to(dtype=pipe.torch_dtype, device=pipe.device) # torch.Size([1, 20, 21, 60, 104])
        return {"y": y}
...
class WanVideoUnit_VACE(PipelineUnit):
    ...
    def process(
        ...
        if vace_video is not None or vace_video_mask is not None or vace_reference_image is not None:
            ...
            return {"vace_context": None, "vace_scale": vace_scale} # None | 1.0
...
class WanVideoUnit_AnimatePoseLatents(PipelineUnit):
    ...
    def process(self, pipe: WanVideoRefinerPipeline, animate_pose_video, tiled, tile_size, tile_stride):
        if animate_pose_video is None:
            return {}
        pipe.load_models_to_device(self.onload_model_names)
        animate_pose_video = pipe.preprocess_video(animate_pose_video) # torch.Size([1, 3, 77, 480, 832])
        pose_latents = pipe.vae.encode(animate_pose_video, device=pipe.device, tiled=tiled, tile_size=tile_size, tile_stride=tile_stride).to(dtype=pipe.torch_dtype, device=pipe.device) # False | None | None -> torch.Size([1, 16, 20, 60, 104])
        return {"pose_latents": pose_latents} # torch.Size([1, 16, 20, 60, 104])
...
class WanVideoUnit_AnimateFacePixelValues(PipelineUnit):
    ...
    def process(self, pipe: WanVideoRefinerPipeline, inputs_shared, inputs_posi, inputs_nega):
        ...
        inputs_nega["face_pixel_values"] = torch.zeros_like(inputs_posi["face_pixel_values"]) - 1 # torch.Size([1, 3, 77, 512, 512])
        return inputs_shared, inputs_posi, inputs_nega
...
def model_fn_wan_video(
    ...
    **kwargs,
    ...
    if sliding_window_size is not None and sliding_window_stride is not None: # None | None
        model_kwargs = dict(
            dit=dit,
    ...
    # Timestep
    if dit.seperated_timestep and fuse_vae_embedding_in_latents: # False | False
        ...
        t = dit.time_embedding(sinusoidal_embedding_1d(dit.freq_dim, timestep)) # 256 | tensor([940.], device='cuda:0', dtype=torch.bfloat16) --->> torch.Size([1, 5120])
        t_mod = dit.time_projection(t).unflatten(1, (6, dit.dim)) # torch.Size([1, 6, 5120])
    
    # Motion Controller
    if motion_bucket_id is not None and motion_controller is not None: # None | None
        t_mod = t_mod + motion_controller(motion_bucket_id).unflatten(1, (6, dit.dim))
    context = dit.text_embedding(context) # torch.Size([1, 512, 4096]) -> torch.Size([1, 512, 5120])

    x = latents # torch.Size([1, 16, 21, 60, 104])
    # Merged cfg
    if x.shape[0] != context.shape[0]: # 1 != 1
        x = torch.concat([x] * context.shape[0], dim=0)
    if timestep.shape[0] != context.shape[0]: # 1 != 1
        timestep = torch.concat([timestep] * context.shape[0], dim=0)

    # Image Embedding
    if y is not None and dit.require_vae_embedding: # torch.Size([1, 20, 21, 60, 104]) | True
        x = torch.cat([x, y], dim=1) # torch.Size([1, 36, 21, 60, 104])
    if clip_feature is not None and dit.require_clip_embedding: # torch.Size([1, 257, 1280]) | True
        clip_embdding = dit.img_emb(clip_feature) # torch.Size([1, 257, 1280]) -> torch.Size([1, 257, 5120])
        context = torch.cat([clip_embdding, context], dim=1) # torch.Size([1, 769, 5120])
        
    # Camera control
    x = dit.patchify(x, control_camera_latents_input) # torch.Size([1, 36, 21, 60, 104]) -> torch.Size([1, 5120, 21, 30, 52])
    ...
    # Reference image
    if reference_latents is not None: # None
    ...
    # TeaCache
    if tea_cache is not None: # None
        tea_cache_update = tea_cache.check(dit, x, t_mod)
    else:
        tea_cache_update = False
        
    if vace_context is not None: # None
        vace_hints = vace(
    ...
    if tea_cache_update: # False
        ...
        for block_id, block in enumerate(dit.blocks):
            # Block
            if vap is not None and block_id in vap.mot_layers_mapping:# None 
            ...
            # Animate
            if pose_latents is not None and face_pixel_values is not None:  # torch.Size([1, 16, 20, 60, 104]) | torch.Size([1, 3, 77, 512, 512])
                x = animate_adapter.after_transformer_block(block_id, x, motion_vec) # torch.Size([1, 32760, 5120]) | torch.Size([1, 21, 5, 5120]) --->> torch.Size([1, 32760, 5120])
        if tea_cache is not None:
            tea_cache.store(x)
            
    x = dit.head(x, t) # torch.Size([1, 32760, 5120]) | torch.Size([1, 5120]) -> torch.Size([1, 32760, 64])
    if use_unified_sequence_parallel: # False
        if dist.is_initialized() and dist.get_world_size() > 1:
    ...
    if reference_latents is not None:
        ...
        f -= 1
    x = dit.unpatchify(x, (f, h, w)) # torch.Size([1, 32760, 64]) | (21, 30, 52) -> torch.Size([1, 16, 21, 60, 104])
    return x
